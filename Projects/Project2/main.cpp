...
int main(int argc, char** argv)
{
  ...
  while (isGameRunning)
  {
    ...
    else if (game.gameState == GameState::Dead)
    {
      ...
      printControlScheme(GameState::Help);
      getAKey(input, WIN32_MODE); //get a key and trash it
      
      cout << "Please try again!" << endl;
      isGameRunning = false;
    }
    else if (game.gameState == GameState::SaveAs)
    {
...
bool fightMonster(GameProperties &game, Asset* monster, Asset* player, bool WIN32_MODE)
{
  ...
  if (monster->hp <= 0)
  {
    ...
    printFight(game, monster, player);
    cout << endl;
    cout << "You are victorious! You have killed the " << monster->name << "!" << endl;
    cout << "You gain " << monster->exp << " experience points." << endl;
